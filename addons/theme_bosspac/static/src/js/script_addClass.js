...
/**
 * this is an attempt to initialise class based inheritence for all the ajax call associated with the other companies
 */
class Product_Management {
	constructor(img_source_url) {
		this.img_source_url = img_source_url;
	}
	CallController() {
		var img_source = this.img_source_url
		random = Math.random();
		odoo.define('web.ajax'+random, function(require){
			...
			var res = ajax.jsonRpc("/pcm/products/browse/"+prod_id, 'call', {
				...
				if(returnval['status'] == 'OK'){
					...
					document.getElementById("image1").src = "/product_shelf/static/src/img/"+img_source+"/"+prod_id+"/1.png";
					document.getElementById("image2").src = "/product_shelf/static/src/img/"+img_source+"/"+prod_id+"/2.png";
					document.getElementById("image3").src = "/product_shelf/static/src/img/"+img_source+"/"+prod_id+"/3.png";
					document.getElementById("image4").src = "/product_shelf/static/src/img/"+img_source+"/"+prod_id+"/4.png";
					document.getElementById("image5").src = "/product_shelf/static/src/img/"+img_source+"/"+prod_id+"/5.png";
					document.getElementById("image6").src = "/product_shelf/static/src/img/"+img_source+"/"+prod_id+"/6.png";
				}
			})
...
//----------------------------------------
// let's initialise the class to achieve call for amazon
//-----------------------------------------
function img_gallery_ajax_amazon(){
	var call_amazon = new Product_Management("amazon");
	call_amazon.CallController()
}
//----------------------------------------
// let's initialise the class to achieve call for newegg
//-----------------------------------------
function img_gallery_ajax_newegg(){
	var call_newegg = new Product_Management("newegg");
	call_newegg.CallController()
}
//----------------------------------------
// let's initialise the class to achieve call for mwave
//-----------------------------------------
function img_gallery_ajax_mwave(){
	var call_mwave = new Product_Management("mwave");
	call_mwave.CallController()
}
//----------------------------------------
// let's initialise the class to achieve call for pcbyte
//-----------------------------------------
function img_gallery_ajax_pcbyte(){
	var call_pcbyte = new Product_Management("pcbyte");
	call_pcbyte.CallController()
}
	...
	odoo.define ('web.ajax77'+random, function(require){
		...
		var res = ajax.jsonRpc("/pcm/prodel/"+idk, "call", {
			...
			if(returnval['status'] == 'OK'){
				...
				$(".drag").remove();
				$("#"+kap).remove();
				$("#myTablei_info").html("Showing 0 to 0 of 0 records");
